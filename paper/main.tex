...
In order to support the three modes of operation desired for our robotic explorer,
we architect F-Prime components as shown in Figure~\ref{fig:arch}.
Each component already provided by F-Prime is printed in green,
and components developed specifically for our mission requirements are in yellow.
...
\digraph{arch}{
...

  {
    ...
    cmd [fillcolor=chartreuse4 shape=rect label="cmdDisp" style=filled fontsize=10 fixedsize=true width=1.1];
    blank2 [label="" shape=plaintext width=2.0]
    prm [fillcolor=chartreuse4 shape=rect label="Prm DB" style=filled fontsize=10 fixedsize=true width=1.1];
  }

  {
    rank=same;

    con [fillcolor=darkgoldenrod3 shape=rect label="Ctrl" style=filled fontsize=10 width=0.7];
...

  {
    ...
    event [fillcolor=chartreuse4 shape=rect label="Event" style=filled fontsize=10 fixedsize=true width=1.1];
  }

  cmd -> blank2 [style=invis];
  blank2 -> prm [style=invis];
  tlms -> blank [style=invis];
  tlm -> event [style=invis];
...
\begin{itemize}
  \item {\bf cmdDisp}: 
  \item {\bf Prm DB}:
  \item {\bf Ctrl}: overall control; issues commands to put the explorer in one of the three operating modes.
  \item {\bf MtrCtrl}: motor control; sets the motor speeds and computes odometry
  \item {\bf AnomDet}: density-estimation anomaly detector implemented with mlpack; receives telemetry from TlmSplit
  \item {\bf HWDrv}: Romi hardware driver; issues hardware commands via I2c to I2CDrv
  \item {\bf IMUSen}: Romi IMU sensor driver; issues hardware commands via I2c to I2CDrv
  \item {\bf I2CDrv}: low-level I2C driver
  \item {\bf TlmSplit}: telemetry splitter; routes all telemetry {\em both} to default TlmChan and also AnomDet
  \item {\bf TlmChan}: telemetry collector
  \item {\bf Event}: event bus
\end{itemize}
...
The basic plan for experiments is this:

\begin{itemize}
  \item First in `stationary' mode, detect unexpected use of CPU resources.
    \subitem This is a really easy example.
    \subitem In fact I even need to point out it could be done with just plain
thresholding.
    \subitem Show a plot of the density estimate over time.

  \item Next in `straight-line' mode, detect when the vehicle starts to turn.
    \subitem This example is more difficult and requires correlations between sensors.
    \subitem Again show a plot of density over time with anomalous regions highlighted.
    \subitem The graph should also show wheel speeds.

  \item Still in `straight-line' mode, detect when physical obstructions cause problems:
    \subitem Half-on-carpet half-on-wood floor will likely result in turning even if the wheels are fine.
    \subitem Running into a cat will result in an anomaly too.  Will the cat stay still?  Unclear.
    \subitem Plots should be the same: motor, important sensors, density estimate.

  \item Now in `spinning' mode, detect when a physical obstruction causes a problem.
    \subitem Put a finger on one wheel.
    \subitem Pick the robot up and put it on its side.
    \subitem Again, plots should be the same.
\end{itemize}

Probably it makes sense for a few of these anomaly situations to include pictures of the anomaly we are applying, too.  Like e.g. a picture of the robot with a finger on the wheel; a picture of the robot interacting with a cat; a picture of the robot on its side.

Details will also be needed about precisely what the `training' of the density estimator involved.  For instance, for `spin' mode I cycle it through all possible wheel speed values for a while.
