...
%These problems can be resolved via the use of open source C++-based
%scientific computing software, such as the mlpack machine learning
%library and Armadillo linear algebra library.  These tools enable a wide
%variety of potential machine learning and AI applications directly on
%the spacecraft; because they are written in lightweight header-only C++,
%they are able to be cross-compiled easily to any target device,
%including typical spacecraft compute architectures such as ARM, RISC-V,
%PowerPC, and SPARC64.  In addition, the use of C++ means minimal
%computational overhead.
%
%To demonstrate the efficacy of this approach, we have implemented the
%first use of C++-based machine learning toolkits directly as components
%in the F-Prime framework, a standard flight software offering from NASA.
%Using telemetry channels that F-Prime makes available, we train an
%anomaly detection model using kernel density estimation with kd-trees.
%This model is significantly more capable than common simple
%threshold-based anomaly detectors, and is adaptive to the current
%working state of the spacecraft via continual retraining of the model.
%
%This demonstration is physically validated on a two-wheeled robotic
%explorer platform; when we cause a slight wheel obstruction, the anomaly
%detector immediately detects the issue.  The demonstration code is
%open-source and well-documented.  It is therefore readily adaptable to
%any other on-spacecraft machine learning task, such as image
%classification, object detection and segmentation, predictive control,
%position and orientation estimation, and other real-time modeling tasks.
