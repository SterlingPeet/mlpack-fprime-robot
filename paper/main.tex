...
\begin{Figure}
\begin{center}
\includegraphics[width=\textwidth]{img/romi_anomaly.png}
\end{center}
...
During this time,
the rover's speed was set alternatingly to positive and negative speed values,
...
the log-density estimate drops precipitiously,
as expected.
...
Lastly, we tested the rover in the spinning mode of operation.
Here, the rover is expected to be stationary but spinning at any speed.
Like straight-line operation,
we trained the density estimate by
running the rover in a clean environment with no obstructions or other problems
for roughly an hour.
During this time,
the rotational speed of the rover was swept through all possible values.

Once the training process was complete,
we operated the rover in spinning mode,
both without anomalies and with anomalies.
We applied TODO anomalies:

\begin{enumerate}
  \itemsep -2pt
  \item Obstruct one wheel
  \item Obstruct both wheels (no rotation)
  \item Set the rover on its side
  \item Cause wheels to have no contact with the ground (no rotation)
  \item Run rover in straight-line mode
\end{enumerate}
